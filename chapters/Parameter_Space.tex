...
\hyperref[sec:informal_local_product]{\thechapter.1\;\;Informal picture: parameter space as a local product space} \dotfill & \pageref{sec:informal_local_product}\\
\hyperref[sec:parammanifold_formal]{\thechapter.2\;\;Formal picture: parameter manifold and bundle viewpoint} \dotfill & \pageref{subsec:parammanifold_formal}\\
\hyperref[sec:sowhat]{\thechapter.3\;\;So what? Geometric quantities and why we care} \dotfill & \pageref{sec:sowhat}\\
\end{tabular}
\end{minipage}
\end{center}
...
We stack the per-datum Jacobians of the model into $J_{\mathcal{D}}(\theta) = \big[\frac{\partial f_\theta(x^{(1)})}{\partial \theta}; \ldots; \frac{\partial f_\theta(x^{(N)})}{\partial \theta}\big] \in \mathbb{R}^{NO \times P}$. Then under mild regularity conditions, for any \(\theta_0\in\Theta\) and \(\theta=\theta_0+\delta\).
By Taylor's theorem,
\begin{equation}
...
\emph{The following sections are fairly technical and perhaps more comprehensive than necessary. Understanding all of the details of how we formalize the intuition sketched above is not necessary for understanding the rest of this thesis. Readers not interested in the formal details can feel free to skip to \Cref{chap:linalg}.}
...
infinitesimal data across such structures, we highly recommend \citep{Sharpe1997}. 
