...
\begin{enumerate}
    ...
    Empirically, a \emph{linearized} predictive model outperforms standard Laplace Approximation, even though linearization seems like an additional approximation. Specifically, the Laplace Approximation is known to underfit on the training data. This chapter explains
    this phenomenon by considering the reparameterizations of the parameter space.

    \item \textbf{Reparameterization-aware Laplace approximation.}
    We develop a generalization of the Laplace Approximation, called \emph{Laplace diffusion}, which outperforms Linearized Laplace despite using the Neural Network predictive model, similar to standard Bayesian Model Averaging.

    \item \textbf{Model selection at scale.}
    We also address the question of choosing a prior precision. We develop a scalable marginal-likelihood optimization algorithm using the matrix-free primitives from \Cref{chap:linalg}. We show that this works for models with millions of parameters.
    \end{enumerate}
\end{keypointsbox}

\begin{center}
\begin{minipage}{0.95\linewidth}
\noindent\textbf{Chapter \thechapter\ -- Laplace Approximation: Underfitting and Laplace Diffusion}\par\vspace{3pt}

\begin{tabular}{@{}p{0.83\linewidth}r@{}}
\hyperref[sec:laplace_linearization_puzzle]{\thechapter.1\;\;The Linearization Puzzle in Laplace Approximation} \dotfill & \pageref{sec:laplace_linearization_puzzle}\\
\hyperref[sec:laplace_diffusion]{\thechapter.2\;\;Laplace Diffusion} \dotfill & \pageref{sec:laplace_diffusion}\\
\hyperref[sec:ld_experiments]{\thechapter.3\;\;Laplace Diffusion Experiments} \dotfill & \pageref{sec:ld_experiments}\\
\hyperref[sec:marginal_likelihood_optimization]{\thechapter.4\;\;Marginal-Likelihood Optimization} \dotfill & \pageref{sec:marginal_likelihood_optimization}
...
So far we have studied the geometry of the parameter space in Bayesian Deep Learning in \cref{chap:setting} and we have developed practical algorithms for making computations in the parameter space equipped with this geometry in \cref{chap:linalg}. In this chapter, we will apply this framework to a standard Approximate Bayesian Inference method, specifically the Laplace Approximation. We will explain how and why Laplace Approximation fails in the Bayesian Deep learning setting and how a simple modification can addresses this failure mode.

\section{The Linearization Puzzle in Laplace Approximation}
...
\begin{wrapfigure}[21]{r}{0.45\textwidth}
  % \vspace{-\baselineskip}
  \centering
  \includegraphics[width=0.9\linewidth]{chapters/ch5_figures/laplace_fails.png}
  \vspace{-2mm}
  \caption{The predictive variance of the Laplace approximation is very high on the training data; by contrast, linearised Laplace has low in-distribution variance and high out-of-distribution variance.}
  \label{fig:laplace_fails}
  \vspace{-\baselineskip}
\end{wrapfigure}
...
log-posterior around a mode $\theta_{\textsc{map}}$, which is our approximate posterior. This results in a Gaussian approximate posterior $\mathcal{N}\left(\theta ~|~ \theta_{\textsc{map}}, \mathbf{H}_{\theta_{\textsc{map}}}^{-1}\right)$, where $\mathbf{H}_{\theta_{\textsc{map}}}$ is the Hessian of the loss function. Typically, the Hessian is replaced by the Generalised Gauss-Newton because the Hessian is not guaranteed to be Positive Definite (see \Cref{chap:background} for a more detailed discussion). This gives us the following approximate posterior:

\begin{align}
\begin{split}
  q(\theta | \mathcal{D}) &= \mathcal{N}\left(\theta ~|~ \theta_{\textsc{map}}, (\textrm{GGN}_{\theta_{\textsc{map}}} + \alpha\mathbb{I})^{-1}\right), \\ %\quad \text{where} \\
  \textrm{GGN}_{\theta_{\textsc{map}}} &= \sum_{i=1}^N \mathbf{J}_{x_i}(\theta_{\textsc{map}})^\top \mathbf{\tilde{H}}(\eta_i)  \mathbf{J}_{x_i}(\theta_{\textsc{map}}) \\
  &= \mathbf{J}_{\mathcal{D}}(\theta_{\textsc{map}})^\top \mathbf{\tilde{H}}(\eta)  \mathbf{J}_{\mathcal{D}}(\theta_{\textsc{map}}).
\end{split}
\end{align}
...
\begin{align}
    p(\mathbf{y}^*|\mathbf{x}^*\!, \mathcal{D}) = \mathbb{E}_{\theta\sim q}[p(\mathbf{y}^*|f^{\theta_{\textsc{map}}}_{\mathrm{lin}}(\theta, \mathbf{x}^*))] %\\
                            \approx \frac{1}{S}\! \sum_{i=1}^S p(\mathbf{y}^*|f^{\theta_{\textsc{map}}}_{\mathrm{lin}}(\theta_i, \mathbf{x}^*)), \quad \theta_i \sim q.
\end{align}
where $f^{\theta_{\textsc{map}}}_{\mathrm{lin}}(\theta, \mathbf{x}^*) = f (\theta_{\textsc{map}}, x) + \mathbf{J}_{x_i} (\theta - \theta_{\textsc{map}})$, is the first order Taylor expansion of the model map around the mode $\theta_{\textsc{map}}$. This is referred to as \emph{linearised Laplace}. 

As seen in \Cref{fig:laplace_fails}, this leads to vastly different predictive behaviour. Sampled Laplace is known to severely \emph{underfit} \citep{lawrence2001variational, immer2021scalable}, whereas the linearised Laplace approximation does not \citealt{immer2021improving}. It is an open problem \emph{why} the crude linearization is beneficial \citep{papamarkou2024position}. This is what we mean by the lienarization puzzle
...
The geometry that we have described in the previous chapters can explain this phenomenon. First, we will give an accessible description of our solution, relying on Taylor approximations, and then restate this solution in more precise geometric language. 


Let $\theta_{\textsc{map}}$ denote the mode (typically MAP), and draw a Laplace perturbation
\[
\theta=\theta_{\textsc{map}}+\delta,
\qquad
\delta \sim \mathcal{N}(0,\Sigma),
\qquad
\Sigma=(\mathrm{GGN}_{\theta_{\textsc{map}}}+\alpha I)^{-1}.
\]
For any input $x$, expand the network map $f(\theta,x)$ around $\theta_{\textsc{map}}$:
\begin{equation}
\label{eq:taylor_single_x}
f(\theta_{\textsc{map}}+\delta,x)
=
f(\theta_{\textsc{map}},x)
+
J_x(\theta_{\textsc{map}})\,\delta
+
r_x(\delta),
\qquad
...
F_{\mathcal{D}}(\theta_{\textsc{map}}+\delta)
=
F_{\mathcal{D}}(\theta_{\textsc{map}})
+
J_{\mathcal{D}}(\theta_{\textsc{map}})\,\delta
+
r_{\mathcal{D}}(\delta).
\end{equation}
The \emph{linearized} model discards the remainder term by construction; i.e.
\[
F^{\theta_{\textsc{map}}}_{\mathrm{lin}}(\theta_{\textsc{map}}+\delta)
=
F_{\mathcal{D}}(\theta_{\textsc{map}}) + J_{\mathcal{D}}(\theta_{\textsc{map}})\delta.
\]
...
Since $\mathrm{GGN}_{\theta_{\textsc{map}}}$ is symmetric positive semidefinite, $\mathbb R^D$ decomposes as an
\emph{orthogonal} direct sum of its image and kernel,
\begin{equation}
\mathbb R^D \;=\; \texttt{im}(\mathrm{GGN}_{\theta_{\textsc{map}}}) \oplus \texttt{null}(\mathrm{GGN}_{\theta_{\textsc{map}}}).
\label{eq:ggn_image_kernel_split}
\end{equation}
Moreover, in the common case where we ignore the output Hessian (e.g.\ Gaussian regression), we have $\mathrm{GGN}_{\theta_{\textsc{map}}}=J_{\mathcal D}(\theta_{\textsc{map}})^\top J_{\mathcal D}(\theta_{\textsc{map}})$ which implies
\begin{align}
\texttt{null}(\mathrm{GGN}_{\theta_{\textsc{map}}})=\texttt{null}\!\big(\mathbf{J}_{\mathcal D}(\theta_{\textsc{map}})\big),\qquad \texttt{im}(\mathrm{GGN}_{\theta_{\textsc{map}}})=\texttt{im}\!\big(\mathbf{J}_{\mathcal D}(\theta_{\textsc{map}})^\top\big)
\end{align}
...
Let $\mathrm{GGN}_{\theta_{\textsc{map}}}=\mathbf{U}^\top \Lambda \mathbf{U}$ be an eigendecomposition and split the eigenvectors as
...
\[
\theta = \theta_{\textsc{map}} + \delta_{\mathrm{im}} + \delta_{\mathrm{ker}},
\qquad
\delta_{\mathrm{im}}\in\texttt{im}(\mathrm{GGN}_{\theta_{\textsc{map}}}),\ \ 
\delta_{\mathrm{ker}}\in\texttt{null}(\mathrm{GGN}_{\theta_{\textsc{map}}}).
\]
...
curvature in directions annihilated by $J_{\mathcal D}(\theta_{\textsc{map}})$.

\paragraph{Linearized Laplace cannot change training logits along Jacobian-kernel directions.}
If we decompose a Laplace samples as $\theta=\theta_{\textsc{map}}+\delta_{\mathrm{im}}+\delta_{\mathrm{ker}}$ with
$\delta_{\mathrm{ker}}\in\Ker(J_{\mathcal D}(\theta_{\textsc{map}}))$, then
\begin{align}
F^{\theta_{\textsc{map}}}_{\mathcal D,\mathrm{lin}}(\theta_{\textsc{map}}+\delta_{\mathrm{im}}+\delta_{\mathrm{ker}})
-
F_{\mathcal D}(\theta_{\textsc{map}}) = J_{\mathcal D}(\theta_{\textsc{map}})\delta_{\mathrm{im}}
\;+\;\underbrace{J_{\mathcal D}(\theta_{\textsc{map}})\delta_{\mathrm{ker}}}_{=\,0}
.
...
-
F_{\mathcal D}(\theta_{\textsc{map}}) = J_{\mathcal D}(\theta_{\textsc{map}})\delta_{\mathrm{im}}
\;+\;\underbrace{J_{\mathcal D}(\theta_{\textsc{map}})\delta_{\mathrm{ker}}}_{=\,0} \;+\; r_\mathcal{D}(\delta_{\mathrm{ker}} + \delta_{\mathrm{im}})
.
...
Given a Riemannian manifold $(\mathcal{M}, \mathbf{G})$, one of the simplest choice of distribution that respects the Riemannian metric $\mathbf{G}$ is a Riemannian diffusion (or Brownian motion, c.f.\@ \citet{hsu2002stochastic}) stopped at time $t$.
 \begin{align}
...
Given a MAP estimate $\theta_{\textsc{map}}\in\Theta$, and parameter space $\Theta$ with a
Riemannian metric $g_\theta = g_\theta^{\mathrm{hor}}\oplus g_\theta^{\mathrm{ver}}$.
Let $(\theta_t)_{t\in[0,1]}$ be the Riemannian Brownian motion on $(\Theta,g)$ started at $\theta_{\textsc{map}}$,
i.e.\ the solution to the SDE in \Cref{eq:rbm_sde} with initial condition $\theta_0=\theta_{\textsc{map}}$.
...
$\theta_0=\theta_{\textsc{map}}$. 
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{chapters/ch5_figures/relu_example_v3.pdf}
  \caption{The \emph{weight space} of a neural network (here Eq.~\ref{eq:relu}) overparametrizes the associated \emph{function space}. This induces families (shown as orange curves) of weights that correspond to the same functions. Model linearization (left) linearizes these families. In the nonlinear setting, Gaussian weight distributions (center) do not adapt to the families, while our proposed diffusion (right) captures the associated invariance with a pseudo-metric.}
  \label{fig:relu}
  \vspace{-2mm}
\end{figure}
The probability distribution evolves according to the Fokker--Planck equation associated with the
Riemannian Brownian motion, which can be written as a heat equation:
\begin{equation}
\partial_t \rho_t \;=\; \tau\,\Delta_g \rho_t,
\qquad
\rho_{t=0} \;=\; \delta_{\theta_{\textsc{map}}},
\label{eq:fokker_planck_heat}
\end{equation}
where $\Delta_g$ is the Laplace--Beltrami operator induced by the metric $g_\theta$,
and $\delta_{\theta_{\textsc{map}}}$ denotes a point mass at the mode. Thus the Laplace diffusion posterior
...
of the heat kernel: for $t\downarrow 0$ and $\theta$ near $\theta_{\textsc{map}}$,
\begin{align}
\rho_t(\theta)
\;\approx\;
\frac{1}{(2\pi t)^{d/2}}
\exp\!\left(-\frac{1}{2t}\,d_g(\theta,\theta_{\textsc{map}})^2\right)
\Big(a_0(\theta,\theta_{\textsc{map}})+\mathcal O(t)\Big),
\label{eq:parametrix}
\end{align}

where $d=\dim(\Theta)$, $d_g$ is the Riemannian distance induced by $g$. 

The key takeaway here is that Laplace diffusion is a local approximate posterior whose
leading behaviour is controlled by $d_g(\theta,\theta_{\textsc{map}})^2$ as opposed to Euclidean distance, as is the case with the standard Laplace Approximation.
This matters in overparameterized models because directions corresponding to reparameterizations (fibres)
can be \emph{curved} in parameter space: using geodesic distance means the approximation spreads mass according to the geometry of those curved reparameterization lines, rather than treating all directions as globally straight. 
This is illustrated on the RELU example in \Cref{fig:relu}. Additionally, in normal coordinates around $\theta_{\textsc{map}}$ we have
\begin{equation}
d_g(\theta_{\textsc{map}}+\delta,\theta_{\textsc{map}})^2
=
\delta^\top G(\theta_{\textsc{map}})\,\delta \;+\; \mathcal O(\|\delta\|^3),
\label{eq:dist_local_quad}
\end{equation}
so the leading term of \eqref{eq:parametrix} is Gaussian with covariance $t\,G(\theta_{\textsc{map}})^{-1}$, which corresponds to the standard Laplace Approximation. 
...
Euler--Maruyama step from $\theta_{\textsc{map}}$ in \Cref{eq:em_update}, we recover the usual Laplace perturbation
$\theta=\theta_{\textsc{map}}+\delta$ with $\delta\sim\mathcal N(0,(\mathrm{GGN}_{\theta_{\textsc{map}}}+\alpha I)^{-1})$,
...
\begin{wrapfigure}[13]{r}{0.6\textwidth}
  \centering
  \vspace{-5mm}
  \includegraphics[width=\linewidth]{chapters/ch5_figures/sample_decomposition_v5.pdf}
  \vspace{-5mm}
  \caption{The \emph{function space} is decomposed into directions of \emph{reparameterizations} (kernel) and \emph{functional change} (non-kernel). We improve the posterior fit by concentrating probability mass on directions of functional change.}
  \label{fig:laplace_decomp}
\end{wrapfigure}
Now we can decompose the predictive variance obtained from sampled Laplace, Linearised Laplace, and Laplace diffusion into their kernel and non-kernel contributions respectively. 
\Cref{fig:laplace_decomp} validates our solution to the Linearisation puzzle by showing that Laplace Approximation underfits precisely because its kernel components underfit. Lalace Diffusion's kernel components are aligned with the reparameterization curves of the predictive function, and thus it alleviates the commonly observed underfitting of the Laplace approximation  \looseness=-1


\section{Experiments}
\label{sec:ld_experiments}
\begin{table}
...
\begin{tabular}{lrrrr}
    ...
    Marginal likelihood (log)
    	& $\uparrow$
    	& -192,757
    	&  \cellcolor[gray]{0.9} \textbf{-154,475}
    	& -876,444 
    	\\
	\midrule
    Joint likelihood: train (log)
    	& $\uparrow$
    	& \cellcolor[gray]{0.9} \textbf{-81.2} $\pm$ \textbf{ 15.4} 
    	&  \cellcolor[gray]{0.9}  \textbf{-81.2} $\pm$ \textbf{ 15.4} 
    	& -5,669.2 $\pm$ 124.1 
    	\\
    Joint likelihood: test (log)
    	& $\uparrow$
    	& \cellcolor[gray]{0.9} \textbf{-66.5} $\pm$\textbf{ 11.2} 
    	& \cellcolor[gray]{0.9} \textbf{-66.5} $\pm$\textbf{ 11.2} 
    	& -5,260.9 $\pm$ 290.2 
    	\\
    Expected calibration error
    	...
    	& 0.5  $\pm$ 0.01
    	& 0.5  $\pm$ 0.01
    	& \cellcolor[gray]{0.9}\textbf{0.2} $\pm$ 0\textbf{.003} 
    	\\
    AUROC (out-of-dist.)
    	& $\uparrow$
    	&\cellcolor[gray]{0.9}{\textbf{0.9}  $\pm$ \textbf{0.03}}
    	& \cellcolor[gray]{0.9}{\textbf{0.9}  $\pm$ \textbf{0.03}}
    	& 0.5 $\pm$ 0.010
    	\\
    \bottomrule
